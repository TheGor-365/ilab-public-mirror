class PagesController < ApplicationController
  ...
  def store
    ...
    # --- Новый блок: фильтр по семейству (iPhone/iPad/MacBook/…)
    @families = Generation.distinct.order(:family).pluck(:family).compact
    @family   = params[:family].presence

    if @family.present?
      base = base.joins(:generation).where(generations: { family: @family })
    end

    # Было: @product = base — это relation; не используем.
