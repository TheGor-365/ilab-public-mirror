class MakeProductSidebarMaterialized < ActiveRecord::Migration[7.1]
  # создаём уникальный индекс CONCURRENTLY → нужен вне транзакции
  ...
  def up
    ...
    execute <<~SQL
      CREATE MATERIALIZED VIEW product_sidebar_entries AS
    ...
    execute <<~SQL
      CREATE UNIQUE INDEX CONCURRENTLY ux_product_sidebar_entries
      ON product_sidebar_entries (product_id, kind, entity_id)
    SQL

    execute "REFRESH MATERIALIZED VIEW CONCURRENTLY product_sidebar_entries"
  end

  def down
    execute "DROP MATERIALIZED VIEW IF EXISTS product_sidebar_entries"
